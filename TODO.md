...
- [x] LoRA 適用 → マージ → 繰り返しのパイプライン構築
  - [x] パイプライン定義（反復ステップ、減衰率のスケジュール）
  - [x] 既存の `lora_ops` 拡張の `apply / merge` 機能をループ内で呼び出す統合
  - [x] 中間モデルの一時保存（ディスク or メモリ、VRAM とのバランス）
- [x] 反復回数・減衰率のパラメータ化
  - [x] 減衰カーブの選択（線形, 指数, コサインアニーリング）
  - [x] 反復ごとのパラメータオーバーライド設定
- [x] XY Plot 風の比較画像グリッド生成（X軸: 反復回数, Y軸: 減衰率 等）
  - [x] 既存の `xyz_plot.py` のグリッド生成ロジックを共通化して再利用
- [x] 各反復ステップのサンプル画像を並べて変化を可視化
- [x] ポイズンマージを複数LoRAでできるようにする

> **[考察]** 毒薬マージは `module/pipeline/poison.py` に実装されました。タスクキューに統合され、減衰アルファでのイテレーション、画像生成、グリッド生成をサポートしています。
...
- [x] Strategy ドロップダウンの追加
- [x] Target Strategy ドロップダウンの追加
- [x] 既存のブロック重み設定との連携確認
  - [x] `mbw_each` 拡張の `pre_config_hook` でストラテジー上書きが正しく伝播するか検証
...
- [x] バッチコマンド構文に `strategy` / `target_strategy` パラメータ追加
  - [x] コマンドパーサー `parse_multi_merge_command()` に新変数を追加
- [x] UI 上での選択肢追加（デフォルト値、行ごとのオーバーライド）
...
- [x] LoRA マージ時の計算戦略選択
- [x] LoRA 抽出時のターゲット戦略選択
- [x] `lora_ops` 拡張の内部ロジックとの整合性確認
...
- [x] 生成設定のプリセット管理
  - [x] 現在の `presets.py` を拡張し、マージ後生成用のプリセットカテゴリを追加
  - [x] プロンプト / ネガティブ / 解像度 / ステップ数 / CFG のセット保存
- [x] 複数プロンプトでの一括生成
  - [x] プロンプトリスト（テキストファイル or UI 入力）からバッチ生成
  - [x] 生成結果のサムネイルギャラリー表示
- [x] 生成結果をヒストリに自動紐付け
  - [x] `post_merge_hook` で画像生成を自動的にトリガー
  - [x] ヒストリの `entry` に生成画像パスを格納
...
- [x] LRU キャッシュによる VRAM 管理
  - [x] `functools.lru_cache` or カスタム LRU（VRAM 容量ベースの eviction）
  - [x] キャッシュヒット率のモニタリング
- [x] キャッシュサイズ上限の設定
  - [x] 設定ファイル or UI でバイト単位の上限指定
  - [x] VRAM / RAM の自動検出と推奨値の提示
- [x] モデルリロードボタン（UI）
  - [x] キャッシュの手動クリア機能
  - [x] 現在のキャッシュ状態（モデル名、サイズ）の表示
...
- [x] ストラテジー選択の統一化（MBW Each, Multi Merge, LoRA Ops）
