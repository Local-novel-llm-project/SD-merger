...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.tokenizer_max_length = (
            self.tokenizer.model_max_length if max_token_length is None else max_token_length + 2
        )
    ...
    def process_caption(self, caption):
        ...
        is_drop_out = (
            is_drop_out or self.dropout_every_n_epochs and self.current_epoch % self.dropout_every_n_epochs == 0
        )
    ...
    def cache_latents(self, vae, cache_batch_size=8):
        ...
        for info in self.image_data.values():
            ...
            if reso not in bucket_images:
                bucket_images[reso] = []
            bucket_images[reso].append(info)

        with torch.no_grad():
            for reso, infos in bucket_images.items():
                for i in tqdm(range(0, len(infos), cache_batch_size), desc=f"caching latents for {reso}"):
                    batch_infos = infos[i : i + cache_batch_size]
                    ...
                    for info in batch_infos:
                        ...
                        if self.flip_aug:
                            image_flipped = image[:, ::-1].copy()
                            images_flipped.append(self.image_transforms(image_flipped))

                    img_tensors = torch.stack(images).to(device=vae.device, dtype=vae.dtype)
                    latents = vae.encode(img_tensors).latent_dist.sample().to("cpu")
                    for j, info in enumerate(batch_infos):
                        info.latents = latents[j]

                    if self.flip_aug:
                        img_tensors_flipped = torch.stack(images_flipped).to(device=vae.device, dtype=vae.dtype)
                        latents_flipped = vae.encode(img_tensors_flipped).latent_dist.sample().to("cpu")
                        for j, info in enumerate(batch_infos):
                            info.latents_flipped = latents_flipped[j]
    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.latents is not None:
                latents = (
                    image_info.latents if not self.flip_aug or random.random() < 0.5 else image_info.latents_flipped
                )
                image = None
            elif image_info.latents_npz is not None:
                latents = self.load_latents_from_npz(image_info, self.flip_aug and random.random() >= 0.5)
...
def add_training_arguments(parser: argparse.ArgumentParser, support_dreambooth: bool):
    ...
    parser.add_argument(
        "--save_last_n_epochs", type=int, default=None, help="save last N checkpoints / 最大Nエポック保存する"
    )
    parser.add_argument(
...
def load_target_model(args: argparse.Namespace, weight_dtype):
    ...
    if load_stable_diffusion_format:
        ...
        pipe = StableDiffusionPipeline.from_pretrained(
            args.pretrained_model_name_or_path, tokenizer=None, safety_checker=None
        )
