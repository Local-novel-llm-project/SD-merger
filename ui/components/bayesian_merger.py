    ...
    try:
        ...
        try:
            if hasattr(optimizer, "optimizer") and hasattr(optimizer.optimizer, "res"):
                ...
                scores = [-(r) for r in optimizer.trials.losses()]
                plot_path = optimizer.output_dir / f"{optimizer.log_name}_plot.png"
                convergence_plot(scores, figname=plot_path, minimise=False)
                logging.info("Generated convergence plot.")
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                with gr.Accordion("生成・スコアリング設定", open=True):
                    prompt = gr.Textbox(
                        label="Prompt", value="1girl, masterpiece, best quality, highly detailed", lines=2
                    )
                    negative_prompt = gr.Textbox(label="Negative Prompt", value="worst quality, low quality", lines=2)

                    with gr.Row():
                        steps = gr.Slider(minimum=1, maximum=150, value=20, step=1, label="Steps")
                        cfg_scale = gr.Slider(minimum=1.0, maximum=30.0, value=7.0, step=0.1, label="CFG Scale")
            ...
            with gr.Column(scale=1):
                with gr.Accordion("最適化設定", open=True):
                    optimiser_choice = gr.Dropdown(
                        choices=["bayes", "tpe", "atpe"], value="bayes", label="Optimizer Type"
                    )
                    init_points = gr.Slider(minimum=1, maximum=50, value=5, step=1, label="Initial Points (Warmup)")
                    n_iters = gr.Slider(minimum=1, maximum=200, value=20, step=1, label="Iterations")

                    save_best = gr.Checkbox(label="Save Best Model after Optimization", value=False)
                    output_name = gr.Textbox(label="Best Model Name", value="best_bayesian_merge")

                btn_run = gr.Button("Run Optimization", variant="primary")
                status = gr.Textbox(label="Status", value="Ready", interactive=False)

        # パラメータ設定 (高度)
        with gr.Accordion("Advanced Parameter Settings (Freeze & Grouping)", open=False):
            gr.Markdown(
                "特定のブロックを固定値にしたり、グループ化して探索次元を減らすことができます。現在はYAMLまたは直書きのサポートを想定しています。"
            )
            custom_params = gr.JSON(
                label="Custom Ranges / Freeze / Group (JSON)", value={"frozen_params": {}, "groups": []}
            )

        def start_optimization(ma, mb, p, np, s, c, sd, w, h, opt, init, iters, sb, out, custom):
            ...
            BACKGROUND_RUNNER = threading.Thread(target=run_optimization_thread, args=(cfg,), daemon=True)
